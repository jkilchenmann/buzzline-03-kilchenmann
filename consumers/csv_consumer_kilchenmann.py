...
def process_message(message: str, rolling_window: deque, window_size: int) -> None:
    ...
    try:
        ...
        # Append age to the rolling window
        rolling_window.append(age)

        # Check for a stall in the age data
        if detect_stall(rolling_window):
            logger.info(
                f"STALL DETECTED at year {year}: Age stable at {age} over last {window_size} readings."
            )
